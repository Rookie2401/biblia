...
| `checksum-sources.mjs` | every git-ignored file under `data/{mam-raw,oshb,gnt,lexicon,bsb}` | `data/build/SOURCE-CHECKSUMS.json` (committed) — pins the exact inputs an audited build used, since those directories track upstream `master` branches and are not themselves committed |
...
**Two fixes from an external re-review of content audit 2 (2026-09-19).** (1) A BDB skeleton entry sometimes lists more than one spelling before its part of speech (a Kt/Qr pair, e.g. `<w>מלוכי</w> Kt, <w>מלִיכוּ</w> Qr`); the fallback that attaches such an entry to an augmented id only ever compared against the *first* one, so a headword matching only the second (rarer) spelling was wrongly treated as belonging to a different word. `bdb.set` now records every leading `<w>` (`e.ws`), and the match checks all of them. A small, named build-time allowlist (`KNOWN_BDB_ID_NO_TEXT` in `build-lexicon-he.mjs`) now also fails the build if any *other* id has a BDB identifier with no rendered text, so a future instance of this class of bug can't ship silently again. (2) A regex meant to filter archaic ("hath") or theophoric ("Yah…") glosses out of name etymologies had its two `\b` word-boundary escapes turned into literal backspace bytes by an earlier shell heredoc edit — invisible on screen, so the filter silently stopped matching without any visible diff. `test/lexicon-quality.test.ts` now scans every `scripts/*.mjs` file for stray control bytes so this class of corruption fails a test run instead of shipping unnoticed.
