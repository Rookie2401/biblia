...
describe('reader words and verse numbers are operable controls', () => {
  ...
  // point even with no literal space there, which — before this was fixed — could wrap "word."
  // into "word" at the end of one line and a stranded "." starting the next. Confirmed live in
  // the browser before being fixed with a nowrap wrapper around the button and its
  // immediately-following punctuation; this locks that in.
  it('a word button and its trailing punctuation share a no-wrap wrapper, so a line can never break between them', () => {
    render(
      <MemoryRouter>
        <GreekChapter verses={[GR]} bookId="Matt" ch={1} statuses={new Map()} words={words(grBook, 4)} selKey="" showNumbers glossLine={false} />
      </MemoryRouter>,
    );
    const btn = screen.getAllByRole('button', { name: 'Βίβλος' })[0];
    const wrapper = btn.parentElement!;
    expect(wrapper.style.whiteSpace).toBe('nowrap');
    expect(wrapper.textContent).toBe('Βίβλος,'); // the trailing punctuation is inside the same wrapper
  });
